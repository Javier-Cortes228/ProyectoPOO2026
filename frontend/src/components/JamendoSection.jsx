...
import { Search, Loader2, Play, Plus, Heart, X } from 'lucide-react';
            ...
            <div className="glass p-6 rounded-2xl border border-white/5 space-y-6">
                <form onSubmit={submit} className="flex gap-4">
                    <div className="relative flex-1">
                        ...
                        <input
                            className="w-full bg-background border border-white/10 rounded-xl pl-12 pr-10 py-3 text-white focus:outline-none focus:border-[#22D3EE] focus:ring-1 focus:ring-[#22D3EE] transition-colors placeholder:text-textSub"
                            value={query}
                            onChange={(event) => setQuery(event.target.value)}
                            placeholder="Busca música por título, artista, género o álbum..."
                        />
                        {query && (
                            <button
                                type="button"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {resultados.map((track) => (
                    <motion.article
                        ...
                        className={`group relative glass rounded-xl p-4 flex gap-4 overflow-hidden border transition-colors ${activeTrackId === track.id ? 'border-[#22D3EE] bg-[#22D3EE]/5' : 'border-white/5 hover:border-white/10'}`}
                        key={track.id}
                    ...
                    >
                        ...
                        <div className="flex flex-col justify-between items-end z-10">
                            <button
                                type="button"
                                className={`p-2 rounded-full transition-all ${favoriteSet?.has(track.id) ? 'text-[#22D3EE] bg-[#22D3EE]/10' : 'text-textSub hover:bg-white/10'}`}
